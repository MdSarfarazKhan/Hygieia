...
public interface BambooJobRepository extends BaseCollectorItemRepository<BambooJob> {
    ...
    BambooJob findBambooJobJob(ObjectId collectorId, String instanceUrl, String jobName);

    @Query(value="{ 'collectorId' : ?0, options.instanceUrl : ?1, enabled: true}")
    List<BambooJob> findEnabledBambooJobJobs(ObjectId collectorId, String instanceUrl);
}
